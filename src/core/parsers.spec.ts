...
describe("xml parser", () => {
	test("parses an XML string into an object", async () => {
		const result = await xml(
			"<root><name>John McCormack</name></root>",
			undefined,
		);
	...
	test("strips format from options before passing to XMLParser", async () => {
		const result = await xml("<root/>", { format: "xml" } as XmlParserOptions);
	...
	test("Single items turns into array when isArray returns true", async () => {
		...
		const root = (result as { root: { item: string[] } }).root;
		expect(root.item).toEqual(["John McCormack"]);
	});
